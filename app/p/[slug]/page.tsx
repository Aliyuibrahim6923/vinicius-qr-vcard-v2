...
type IconName = "call" | "whatsapp" | "email" | "website" | "linkedin" | "save";
...
export default async function Profile({ params }: Props) {
  ...
  const hasActions = canCall || canWhatsApp || Boolean(employee.email || employee.website || employee.linkedin_url);

  return <main className="profile-page"><article className="profile-card"><header className="profile-brand"><div className="profile-wordmark"><span className="profile-monogram" aria-hidden="true">VG</span><span><strong>VINICIUS GROUP</strong><small>Digital contact</small></span></div><span className="verified-badge">Official</span></header><section className="profile-top">{employee.photo_url ? <Image className="photo" src={employee.photo_url} alt={name} width={128} height={128} priority /> : <div className="photo placeholder" aria-label={`${name} initials`}>{employee.first_name[0]}{employee.last_name[0]}</div>}<h1>{name}</h1><p className="role">{employee.job_title}</p>{employee.department ? <p className="department">{employee.department}</p> : null}{employee.bio ? <p className="bio">{employee.bio}</p> : null}</section><section className="profile-contact" aria-label="Contact actions"><div className="profile-section-heading"><h2>Contact</h2><span>Tap an option</span></div>{hasActions ? <div className="contact-actions">{canCall ? <a href={`tel:${employee.phone}`}><span className="action-icon"><ActionIcon name="call" /></span><span><strong>Call</strong><small>{employee.phone}</small></span></a> : null}{canWhatsApp ? <a href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noopener noreferrer"><span className="action-icon whatsapp"><ActionIcon name="whatsapp" /></span><span><strong>WhatsApp</strong><small>Send a message</small></span></a> : null}{employee.email ? <a href={`mailto:${employee.email}`}><span className="action-icon"><ActionIcon name="email" /></span><span><strong>Email</strong><small>{employee.email}</small></span></a> : null}{employee.website ? <a href={employee.website} target="_blank" rel="noopener noreferrer"><span className="action-icon"><ActionIcon name="website" /></span><span><strong>Website</strong><small>Open website</small></span></a> : null}{employee.linkedin_url ? <a href={employee.linkedin_url} target="_blank" rel="noopener noreferrer"><span className="action-icon"><ActionIcon name="linkedin" /></span><span><strong>LinkedIn</strong><small>View profile</small></span></a> : null}</div> : <p className="no-contact-actions">No direct contact options have been published.</p>}</section>{employee.address ? <div className="detail"><span>Office</span><p>{employee.address}</p></div> : null}<footer className="profile-footer"><a className="button save-contact" href={`/p/${slug}/vcard`}><ActionIcon name="save" /> Save contact</a><p className="privacy-note">Verified by Vinicius Group</p></footer></article></main>;
}
