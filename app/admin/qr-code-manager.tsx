...
export function QRCodeManager({ initialQRCodes, employees }: { initialQRCodes: QRCodeRecord[]; employees: Employee[] }) {
  ...
  async function save(formData: FormData) {
    ...
    if (!response.ok || "error" in result) { setError("error" in result ? result.error : "Unable to save QR code"); return; }
  ...
  async function toggle(qr: QRCodeRecord) {
    ...
    else setError("error" in result ? result.error : "Unable to update QR code");
  }
